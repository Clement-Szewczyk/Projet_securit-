...
def final(request):
    ...
    if user.games_completed < 7 and user.games_completed > 8:
        ...
        return render(request, "fin/triche.hml")
    ...
    if user.games_completed == 8:
        ...
        phrase = " a validé"
        return render(request, "fin/final.html", {'email': email, 'name': premon, 'nom': nom, 'phrase': phrase})
    else:
        ...
        if user.games_completed == 8:
            ...
            return render(request, "fin/triche.html")
...
def export(request):
    ...
    writer.writerow(['email', 'first_name', 'last_name', 'birth_date',
                    'validated', 'games_completed'])
    for user in student.objects.all().values_list('email', 'first_name', 'last_name', 'birth_date', 'validated', 'games_completed'):
